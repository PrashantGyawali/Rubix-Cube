export class AnimationQueue {
  ...
  */
    ...
    process() {
        ...
        const clearanceSpeed=Math.min(1+(this.queue.length/8),10);
        ...
        const animateStep = () => {
            ...
            // Step rotation
            const delta = 0.03 * animation.speed * clearanceSpeed;
            this.rotationGroup.rotation[axis] += Math.min(delta, animation.rotationAngle) * (animation.clockwise ? 1 : -1);
            animation.rotationAngle -=Math.abs(delta);
        };
